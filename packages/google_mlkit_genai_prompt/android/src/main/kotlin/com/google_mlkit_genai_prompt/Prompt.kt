...
class Prompt(private val context: Context) : MethodChannel.MethodCallHandler {
    ...
    override fun onMethodCall(
        ...
        when (call.method) {
            CHECK_FEATURE_STATUS -> checkFeatureStatus(call, result)
            DOWNLOAD_FEATURE -> downloadFeature(call, result)
            RUN_INFERENCE -> runInference(call, result)
    ...
    private fun initialize(call: MethodCall): Any {
        return try {
            val generationInstance = Class.forName("com.google.mlkit.genai.prompt.Generation")
                .getField("INSTANCE").get(null)
            val  generativeModel = generationInstance?.javaClass
                ?.getMethod("getClient")
                ?.invoke(generationInstance)
            Class.forName("com.google.mlkit.genai.prompt.GenerativeModelFutures")
    ...
    private fun checkFeatureStatus(call: MethodCall, result: MethodChannel.Result) {
        ...
        try {
            @Suppress("UNCHECKED_CAST")
             val future = generativeModel.javaClass
    ...
    private fun downloadFeature(call: MethodCall, result: MethodChannel.Result) {
        val id = call.argument<String>("id") ?: return result.error("PromptError", "Missing id", null)
        val generativeModel = instances.getOrPut(id) { initialize(call) }
        ...
        try {
            generativeModel.javaClass
                ...
                .invoke(generativeModel, object: DownloadCallback {

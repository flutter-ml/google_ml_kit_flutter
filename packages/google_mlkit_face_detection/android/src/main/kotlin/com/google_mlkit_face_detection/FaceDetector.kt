...
class FaceDetector(private val context: Context,) : MethodChannel.MethodCallHandler {
    ...
    private fun handleDetection(call: MethodCall, result: MethodChannel.Result) {
        ...
        val detector = instances.getOrPut(id) {
        ...
        detector.process(inputImage)
            .addOnSuccessListener { visionFaces ->
                val faces = visionFaces.map { face ->
                    buildMap<String, Any?> {
    ...
    private fun parseOptions(options: Map<String, Any>) : FaceDetectorOptions {
        val classification = if (options["enableClassification"] as Boolean)
            FaceDetectorOptions.CLASSIFICATION_MODE_ALL
        else
            FaceDetectorOptions.CLASSIFICATION_MODE_NONE

        val landmark = if (options["enableLandmarks"] as Boolean)
            FaceDetectorOptions.LANDMARK_MODE_ALL
        else
            FaceDetectorOptions.LANDMARK_MODE_NONE

        val contours = if (options["enableContours"] as Boolean)
    ...
    private fun getLandmarkData(face: Face): Map<String, DoubleArray?> = mapOf(
    ...
    private fun getContourData(face: Face): Map<String, List<DoubleArray>?> = mapOf(
    ...
    private fun landmarkPosition(face: Face, landmarkInt: Int) : DoubleArray? = face.getLandmark(landmarkInt)?.position?.let { doubleArrayOf(it.x.toDouble(), it.y.toDouble())}

    private fun contourPosition(face: Face, contourInt: Int) : List<DoubleArray>? = face.getContour(contourInt)?.points?.map { doubleArrayOf(it.x.toDouble(), it.y.toDouble())}

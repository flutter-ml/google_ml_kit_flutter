...
public class GoogleMlKitDigitalInkRecognitionPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
            ...
            let unimplemented = FlutterError(
                code: "UNIMPLEMENTED",
                message:
                    "MLKitDigitalInkRecognition is not available via Swift Package Manager on iOS",
                details: nil
            )
            switch call.method {
            case "vision#closeDigitalInkRecognizer":
        ...
        private func handleDetection(call: FlutterMethodCall, result: @escaping FlutterResult) {
            ...
            guard let identifier = DigitalInkRecognitionModelIdentifier(forLanguageTag: modelTag)
            else {
                result(
                    FlutterError(
                        code: "invalid_model", message: "Invalid language tag: \(modelTag)",
                        details: nil))
                return
            }
            ...
            guard let strokeList = args["ink"] as? [String: Any],
                let strokesData = strokeList["strokes"] as? [[String: Any]]
            else {
                result(
                    FlutterError(code: "invalid_args", message: "Missing ink data", details: nil))
                return
            }
            ...
            if let ctx = contextMap {
                ...
                context = DigitalInkRecognitionContext(
                    preContext: preContext, writingArea: writingArea)
            }

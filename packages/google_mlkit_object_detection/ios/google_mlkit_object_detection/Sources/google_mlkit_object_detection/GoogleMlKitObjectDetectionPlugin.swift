...
public class GoogleMlKitObjectDetectionPlugin: NSObject, FlutterPlugin {
        ...
        case "vision#manageFirebaseModels":
                ...
                result(
                    FlutterError(
                        ...
                        message:
                            "Add the GoogleMLKit/LinkFirebase pod to your Podfile to use Firebase-hosted models.",
                        details: nil
                    ))
            #endif
        default:
    ...
    private func handleDetection(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        guard let image = VisionImage.visionImage(from: imageData) else {
            result(
                FlutterError(
                    code: "invalid_image", message: "Invalid or missing image data", details: nil))
            return
        }
    ...
    private func createDetector(from dictionary: [String: Any], result: @escaping FlutterResult)
        ...
        case "local":
            guard let options = getLocalOptions(dictionary) else {
                result(
                    FlutterError(
                        code: "invalid_args", message: "Missing path for local model", details: nil)
                )
                return nil
            }
    ...
    private func getDefaultOptions(_ dictionary: [String: Any]) -> ObjectDetectorOptions {
        ...
        options.detectorMode =
            mode == 0 ? ObjectDetectorMode.stream : ObjectDetectorMode.singleImage
        options.shouldEnableClassification =
            (dictionary["classify"] as? NSNumber)?.boolValue ?? false
        options.shouldEnableMultipleObjects =
            (dictionary["multiple"] as? NSNumber)?.boolValue ?? false
        return options
    }
